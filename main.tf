...
resource "azurerm_role_assignment" "main_aks_system_identity_network_contributor" {
  count = var.use_azure_cni && !var.use_user_assigned_cluster_identity ? 1 : 0
...
resource "azurerm_role_assignment" "main_aks_user_identity_private_dns_zone_contributor" {
  count = var.use_user_assigned_cluster_identity && var.enable_private_cluster && var.private_dns_zone_id != "System" ? 1 : 0
...
resource "azurerm_role_assignment" "main_aks_user_identity_kubelet_identity_contributor" {
  count = var.use_user_assigned_cluster_identity && var.use_user_assigned_kubelet_identity ? 1 : 0

  principal_id         = coalesce(var.cluster_identity["principal_id"], "UNUSED")
  scope                = coalesce(var.kubelet_identity["id"], "/subscriptions/UNUSED")
